#ver0.1 RPi-GP40를 사용한 아날로그 입력 읽기 및 CSV 저장 예제 (2보드 확장 버전)
#ver0.2 IO 출력 입력 기능 추가 _ 조수제 250611
#ver0.25 Config기능 추가 _ 조수제 250611
#ver0.3 DAQ, IO SVID 추가
...
def log_adc_csv():
    ...
    try:
        ...
        daq_state = 0  #★ 파일 열기 실패 시 DAQ 상태 0
        print("[오류] CSV 파일 열기 실패")
        return
    ...
    try:
        while True:
            ...
            row = [timestamp, daq_state] + input_vals  #★ DAQ 상태 포함
        ...
        daq_state = 0  #★ 예외 발생 시 DAQ 상태를 0으로 설정
        print("종료합니다.")
    finally:
        f.close()
